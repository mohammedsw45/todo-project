...
export default function Home(){
   ...
   const { user } =  useContext(AuthContext);
    ...
    useEffect(() => {
      ...
      const file_name = `${task.id + ' - ' + new Date().toLocaleString("en-GB", { day: '2-digit', month: '2-digit', year: "numeric"}) +' - ' + task.title +'.pdf'}`
      // doc.output('save', file_name);
      // Get the PDF data as a string
      doc.setProperties({
        title: `${task.id + ' - ' + new Date().toLocaleString("en-GB", { day: '2-digit', month: '2-digit', year: "numeric"}) +' - ' + task.title +'.pdf'}`,
        subject: 'My PDF Document',
    });
    const pdfDataUri = doc.output('bloburi', {title: `${file_name}.pdf`});
    // window.open(doc.output('bloburi', {filename: `${name} pos.pdf`}), '_blank') // open in a new tab but save with blob name


    window.open(pdfDataUri, '_blank') // open in a new tab but save with blob name
      ...
      //   // Generate the PDF file and trigger the download
                ...
                <div className="tasks">
                    ...
                    filteredtasks.map((task,index) => {
                        ...
                        <div className="task task-content">
                          ...
                          <Tooltip title="Export Task to .pdf">
                          ...
                          <Tooltip title="Delete Task">
                            <img onClick={() => handleDeleteTask(task.id)} src={deleteIcon} className="icon"/>         
                              ...
                              <Popup position="right center" trigger= {<Tooltip title="Add a Step to the Task"><img src={addIcon} className="icon"/></Tooltip>} modal nested> 
                                <div className="popup">
                              ...
                              trigger= {<Tooltip title="Edit Task"><img src={editIcon} className="icon"/></Tooltip>}
